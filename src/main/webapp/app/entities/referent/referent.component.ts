...
export class ReferentComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number): void {
    ...
    if (this.currentSearch) {
      this.referentService
        ...
        .subscribe(
          (res: HttpResponse<IReferent[]>) => this.onSuccess(res.body, res.headers, pageToLoad),
          () => this.onError()
        );
      return;
    }
  ...
  registerChangeInReferents(): void {
    this.eventSubscriber = this.eventManager.subscribe('referentListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IReferent[] | null, headers: HttpHeaders, page: number): void {
