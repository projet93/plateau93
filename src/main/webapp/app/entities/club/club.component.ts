...
export class ClubComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number): void {
    ...
    if (this.currentSearch) {
      this.clubService
        ...
        .subscribe(
          (res: HttpResponse<IClub[]>) => this.onSuccess(res.body, res.headers, pageToLoad),
          () => this.onError()
        );
      return;
    }
  ...
  registerChangeInClubs(): void {
    this.eventSubscriber = this.eventManager.subscribe('clubListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IClub[] | null, headers: HttpHeaders, page: number): void {
